 ...
 ************************************************************/
-- Variante 1: Impedir a eliminação de actores por completo 
-- No presente exemplo, o trigger impede a eliminação e devolve uma mensagem de erro
IF EXISTS (SELECT *
           FROM   sys.objects
...
BEGIN
	...
	ELSE
	BEGIN
		RAISERROR ('Para efeito de preservação de histórico, não se pode apagar actores',10, 1)
		ROLLBACK
	END
END
...
-- Variante 2: Impedir a eliminação de actores com filmes associados     
IF EXISTS (SELECT *
           FROM   sys.objects
           WHERE  object_id = OBJECT_ID(N'[dbo].[tr_NoDelActor]')
                  AND type = N'TR')
...
BEGIN
     ...
     BEGIN
          ROLLBACK
          RAISERROR('Para manter a integridade dos dados, não se pode apagar actores com filmes associados', 16, 1)
          RETURN
     END  
END;

ALTER TABLE dbo.actors
     ALTER COLUMN movieID INT NULL;   

SELECT * 
     FROM   dbo.actors 
     WHERE  movieID IS NULL;

INSERT  INTO dbo.actors (actorID, actorName, actorGender, movieID)
     VALUES (113, 'A13', 'F',13);

INSERT  INTO dbo.actors (actorID, actorName, actorGender, movieID)
     VALUES (200, 'Actor Sem Filmes', 'F',NULL);

SELECT COUNT (*) 
         FROM   dbo.actors 
         WHERE  actorID = 113 AND 
               movieID IS NOT NULL 
